...
namespace Scheduler.Desktop.ViewModels
{
    public class MainViewViewModel : ViewModelBase
    {
        ...
        private void GetSearchTerms()
        {
            ...
            var file = GetAssemblyResource("avares://Scheduler.Desktop/Assets/CoreControlsGroups.json");
            ...
            for (int i = 1; i < coreControls.Count; i++)
            {
                ...
                {
                    MainSearchItems.Add(new MainAppSearchItem(item.Trim(), Type.GetType($"Scheduler.Desktop.Pages.{coreControls[i].PageType}")));
                }
            }

            // Get all FluentAvalonia pages
            file = GetAssemblyResource("avares://Scheduler.Desktop/Assets/FAControlsGroups.json");
            ...
            foreach (var group in controls)
            {
                foreach (var ctrl in group.Controls)
                {
                    ...
                    {
                        MainSearchItems.Add(new MainAppSearchItem($"{ctrl.Header} (FluentAvalonia)", Type.GetType($"Scheduler.Desktop.Pages.{ctrl.PageType}")));
                    }
                    else
                    {
                        MainSearchItems.Add(new MainAppSearchItem(ctrl.Header, Type.GetType($"Scheduler.Desktop.Pages.{ctrl.PageType}")));
                    }
