...
class TestScheduleRule(TestCase):
    ...
    @patch('src.showtimes.domain.time_interval.time_interval.TimeInterval')
    @patch('src.showtimes.domain.schedule.schedule_rule.schedule_rule_id_generator.ScheduleRuleIdGenerator')
    @patch('src.showtimes.domain.schedule.schedule_rule.date_rule.DateRule')
    def test_create(
            self,
            time_interval_mock,
